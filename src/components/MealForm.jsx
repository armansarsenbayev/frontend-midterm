...
const MealForm = ({ onAddMeal }) => {
  ...
  return (
    <div className="card meal-form-container">
      <h3>➕ Add Meal</h3>
      {error && <p style={{ color: '#ff4d4f', fontSize: '0.9rem', marginBottom: '10px', textAlign: 'center' }}>{error}</p>}
      <form onSubmit={handleSubmit} className="meal-form">
        ...
        <div className="macros-row">
          <input type="number" placeholder="P (g)" value={protein} onChange={(e) => setProtein(e.target.value)} className="weight-input" />
          <input type="number" placeholder="F (g)" value={fat} onChange={(e) => setFat(e.target.value)} className="weight-input" />
          <input type="number" placeholder="C (g)" value={carbs} onChange={(e) => setCarbs(e.target.value)} className="weight-input" />
        </div>
        ...
        <button type="submit" className="my-btn">Add to Log</button>
      </form>
    </div>
  );
};
